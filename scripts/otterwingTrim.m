function [M,K,Z,Qip,f,CRv,CRd,EI,GK] = otterwingTrim(B, l, b, t, ba, mhinge, rhop, E, G, nelem, dpm, Ac,hspar)
...
% (c) 2004-2016 Dan Borglund <dodde@kth.se> and David Eller <dlr@kth.se>
  ...
  ca(0.25*nsup+1:0.75*nsup) = 0;% accounts for ailerons 
...
%   small thickness approximation (compare with data sheets)
  ...
  % tapered spar
  l_e = l/(nelem);
  tspar = 0.003; % change it to 0.005 when performing trim for constant Ac
...
%       % attack between main wing and tail plane (I guess)
